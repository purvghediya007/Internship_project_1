...
exports.createIssue = async (req, res) => {
  try {
    ...
    // 🔥 Inject building-specific models safely
    const connection = await getConnection(user.building);
    ...
    /* ================= BASE FLOOR PLAN (FIX) ================= */
    const floorPlan = await FloorPlanDB.findOne({
      floorNumber: user.floorNumber,
      $or: [
        { officeNumber: user.officeNumber }, // office-specific
        { officeNumber: null },              // floor-level
...
exports.getAllIssues = async (req, res) => {
  try {
    ...
    // ✅ Register models safely (important!)
    const IssueDB =
      connection.models.Issue ||
      connection.model("Issue", Issue.schema);
    ...
    const issues = await IssueDB.find()
      .populate({
        ...
        select: "email floorNumber officeNumber"
      })
      .sort({ createdAt: -1 });
